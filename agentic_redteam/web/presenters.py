...
def wizard_view(catalog: Catalog) -> dict:
    ...
    presets = [
        {"id": p.id, "title": p.title, "framework": p.framework,
         "plugin_count": len(p.plugins), "recommended_strategies": p.recommended_strategies}
        for p in catalog.presets.values()
    ]
    groups = {
        group: [
            {"id": pl.id, "name": pl.name, "severity": pl.severity.value,
             "runnable": pl.runnable, "runnable_reason": pl.runnable_reason,
             "strategy_exempt": pl.strategy_exempt, "plugin_type": pl.plugin_type.value}
            for pl in plugins
        ]
