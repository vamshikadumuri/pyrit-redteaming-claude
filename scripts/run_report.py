...
async def _make_llm(system: str, user: str) -> str:
    ...
    req = urllib.request.Request(
        f"{_ATTACKER_ENDPOINT}/chat/completions", data=payload,
...
async def main():
    ...
    target = ModelConfig(endpoint=_GATEWAY, model_name=_GATEWAY_MODEL,
                         api_key_env="OPENAI_CHAT_KEY")
    attacker = ModelConfig(endpoint=_ATTACKER_ENDPOINT, model_name=_ATTACKER_MODEL,
                           temperature=1.0)
