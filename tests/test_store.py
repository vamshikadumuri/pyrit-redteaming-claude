...
async def test_create_run_is_pending_and_listed(store):
    ...
    assert [r["run_id"] for r in await store.list_runs()] == ["run-1"]


async def test_set_status_and_save_summary(store):
    await store.create_run(_request("run-1"))
