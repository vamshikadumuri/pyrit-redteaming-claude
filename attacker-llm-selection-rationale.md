# Choosing the "Attacker" AI Model — Plain-Language Rationale
...
## Quick background (for anyone reading this)
...
| **2** | **Dolphin 3.0 (Llama 3.1 8B, or Mistral 24B)** | **High-volume, low-cost.** Lightweight, uncensored, good at following technical prompts — ideal for generating many test prompts cheaply. | https://huggingface.co/cognitivecomputations/Dolphin3.0-Llama3.1-8B  •  Mistral variant: https://huggingface.co/huihui-ai/Dolphin3.0-R1-Mistral-24B-abliterated |
| **3** | **Qwen3.5 abliterated** (e.g. Huihui-Qwen3.5-35B-A3B; larger: 122B-A10B) | **Hard, app-specific probes.** Strong reasoning on a current-generation base — best when a test prompt must be tailored closely to what our app actually does. Replaces the older Qwen2.5 build. (Abliterated — watch for occasional drift.) | https://huggingface.co/huihui-ai/Huihui-Qwen3.5-35B-A3B-abliterated |
...
> **Why four, not five?** The aim is the best non-overlapping set, not a fixed count. These four already cover every axis that matters — base families (Llama, Mistral, Qwen), both methods (fine-tuned + abliterated), and sizes from 8B to 405B. A fifth "variety" model would have been a near-duplicate, so we get that variety instead through sampling settings (see below) rather than another download to maintain.
...
>
> **Link-rot warning:** community uncensored/abliterated repos (especially experimental ones like huihui-ai's) get renamed, replaced, or deleted frequently. **Open each link in a browser to confirm it's live before relying on it** — a link that worked last week may 404 today. Prefer stable, high-traffic maintainers (e.g. NousResearch, mlabonne) where possible.
...
| 3 | **Qwen3.5 abliterated** (Huihui-Qwen3.5-35B-A3B) | Qwen3.5-35B-A3B | **Abliterated** (refusal direction surgically removed) by huihui-ai on a current-generation base. Raw — no separate repair pass. Abliteration quality varies by method, so test it. | Surgical edit (modern proof-of-concept) |
...
**Plain takeaway:** three of the four picks (#1 Hermes 4, #2 Dolphin 3.0, #4 Dolphin X1) are **retrained-to-comply** models — they keep their skills and follow instructions, which is exactly what we need most. The one **abliterated** model (#3) is included for extra reasoning on a current base; since it's raw (no healing pass), test it carefully for instruction-drift before relying on it.

> Note on the #2 Mistral link: the build linked in the table (`Dolphin3.0-R1-Mistral-24B-abliterated`) is the *reasoning* variant with abliteration added on top. The plain Dolphin 3.0 Mistral (fine-tune only, no abliteration) is the cleaner choice if you don't need step-by-step reasoning — pick it from the Dolphin 3.0 collection.
